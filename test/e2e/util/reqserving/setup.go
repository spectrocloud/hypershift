			...
			for pairNumber := range requestServingPairCount {
				...
				if _, err := controllerutil.CreateOrUpdate(ctx, updateClient, ms, func() error {
					...
					ms.Spec.Template.Spec.ObjectMeta.Labels = map[string]string{
						"hypershift.openshift.io/control-plane":              "true",
						"hypershift.openshift.io/request-serving-component":  "true",
						"hypershift.openshift.io/cluster-size":               size,
						"hypershift.openshift.io/request-serving-gomemlimit": goMemLimit[size],
						"osd-fleet-manager.openshift.io/paired-nodes":        fmt.Sprintf("pair-%d", pairNumber),
					}
					ms.Spec.Template.Spec.Taints = []corev1.Taint{
						{
							Key:    "hypershift.openshift.io/control-plane",
					...
					if err != nil {
						return fmt.Errorf("failed to marshal the machinesset provider spec: %w", err)
					}
